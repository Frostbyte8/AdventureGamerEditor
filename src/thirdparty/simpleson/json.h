...
namespace json
{
	...
	namespace parsing
	{
        ...
        // [FB] So we can use String Stream over snprintf
        static std::ostringstream formatStream("\0");
		...
		std::string get_number_string(const T &number, const char *format)
		{
            ...
            */
            ...
            // [FB] snprintf isn't part of the C++98 standard, so use a string
            // stream instead.

            formatStream << number << '\0';
            std::string result(formatStream.str().c_str());
            formatStream.seekp(std::ios::beg);
