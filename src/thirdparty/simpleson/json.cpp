...
const char* json::parsing::tlws(const char *input)
{
    ...
    // [FB] Add support for c-style comments that were added in the V5 Spec.
    
    const int COMMENT_TYPE_NONE     = -1;
    const int COMMENT_TYPE_SINGLE   = 0;
    const int COMMENT_TYPE_MULTI    = 1;

    int commentLevel    = 0;
    int commentType     = COMMENT_TYPE_NONE;
    
    while(!EMPTY_STRING(output)) 
    {

        // Are we in a comment block?
        if(commentType == COMMENT_TYPE_NONE)
        {
            if(std::isspace(*output))
            {
                commentLevel = 0;
            }
            else if(commentLevel == 0 && *output == '/')
            {
                commentLevel++;
            }
            else if(commentLevel == 1 && (*output == '/' || *output == '*'))
            {
                commentLevel++;
                if(*output == '/')
                {
        ...
        {
            // Single line comments are terminated at a line feed character
            if(commentType == COMMENT_TYPE_SINGLE && (*output == '\r' || *output == '\n'))
            {
                commentLevel = 0;
            ...
            {
                // Meanwhile, we need to find matching */'s for multiline
                // comments
                if(commentLevel == 2 && *output == '*') {
                    commentLevel--;
                }
                else if(commentLevel == 1 && *output == '/') {
                    commentLevel = 0;
                    commentType = COMMENT_TYPE_NONE;
                }
                else {
